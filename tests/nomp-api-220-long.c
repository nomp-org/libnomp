...
void foo(long *a, long *b, int n) {
  static int id = -1;
  const char *knl = "void loopy_kernel(long *a, long *b, int n) {           \n"
  ...
  int err = nomp_jit(&id, knl, annotations, clauses, 3, "a,b,n", NOMP_PTR,
                     sizeof(long), a, NOMP_PTR, sizeof(long), b, NOMP_INTEGER,
                     sizeof(int), &n);
  nomp_chk(err);
...
int main(int argc, char *argv[]) {
  ...
  err = nomp_update(a, 0, n, sizeof(long), NOMP_TO);
  nomp_chk(err);
  err = nomp_update(b, 0, n, sizeof(long), NOMP_TO);
  nomp_chk(err);

  foo(a, b, n);

  err = nomp_update(a, 0, n, sizeof(long), NOMP_FROM);
  nomp_chk(err);
