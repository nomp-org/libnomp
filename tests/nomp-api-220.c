...
void foo(double *a, double *b, int n) {
  static int id = -1;
  const char *knl = "void loopy_kernel(double *a, double *b, int n) {       \n"
...
int main(int argc, char *argv[]) {
  ...
  err = nomp_update(a, 0, n, sizeof(double), NOMP_TO);
  nomp_chk(err);
  err = nomp_update(b, 0, n, sizeof(double), NOMP_TO);
  nomp_chk(err);
