...
int log_match(char *log, char *pattern) {
  regex_t regex;
  int result;
  char buf[BUFSIZ];
...
int main(int argc, char *argv[]) {
  ...
  char *desc;
  err = nomp_get_log(&desc, err, NOMP_ERROR);
  int matched = log_match(
      desc, "Error:.*/libnomp/src/nomp.c:[0-9]* Nomp is not initialized.");
  nomp_assert(!matched);
  ...
  matched = log_match(
      desc,
      "Error:.*/libnomp/src/nomp.c:[0-9]* libnomp is already initialized to "
