...
static int vec_init(int N, int off, double *a, double *b) {
  ...
  err = nomp_run(_nomp_lpy_knl_hndl, 1, _nomp_lpy_knl_gsize,
                 _nomp_lpy_knl_lsize, 4, NOMP_SCALAR, &N, sizeof(N),
                 NOMP_SCALAR, &off, sizeof(off), NOMP_PTR, a, NOMP_PTR, b);
  nomp_chk(err);

  err = nomp_map(a, 0, N + off, sizeof(double), NOMP_D2H);
  nomp_chk(err);
