...
int main(int argc, char *argv[]) {
  ...
  err = nomp_update(a, 0, 10, sizeof(float), NOMP_FROM);
  nomp_assert(err == NOMP_INVALID_MAP_PTR);

  // Mapping H2D multiple times is not an error
  err = nomp_update(b, 0, 10, sizeof(float), NOMP_TO);
  nomp_chk(err);
  err = nomp_update(b, 0, 10, sizeof(float), NOMP_TO);
  nomp_chk(err);

  err = nomp_update(a, 0, 10, sizeof(float), NOMP_TO);
  nomp_chk(err);
  ...
  err = nomp_update(a, 0, 10, sizeof(float), NOMP_FREE);
  nomp_chk(err);
  err = nomp_update(b, 0, 10, sizeof(float), NOMP_FREE);
  nomp_chk(err);
