...
def create_kernel_fun(knl):
    ...
    ndim=knl.default_entrypoint.get_grid_size_upper_bounds_as_exprs( knl.callables_table)[0]
    knl_func = c.FunctionDeclaration(c.Value("", knl_name),knl_args)
    args=[c.Value("sycl::queue", "queue"),c.Value(f"sycl::nd_range<{len(ndim)}>", "nd_range"),c.Value("unsigned int", "nargs"),c.Value("void", "**args")]
    func=c.FunctionBody(c.FunctionDeclaration(c.Value('extern "C" void', f"kernel_function_{len(ndim)}"),args),
    c.Block(([knl_func
        ])))
    return str(func)


# func=create_kernel_fun("foo",3)
# print(func)